...
dat <- read_csv(file = "za_dat.csv")

data <- tibble(
  y = log(dat$RGDP[-1]),
  pi = diff(log(dat$CPI)) * 100,
  i = (1 + (dat$repo[-1] / 100)) ^ 0.25 - 1
  )
plot.ts(data)
...
dum92 <- rep(0, length(data$y))
dum92[11] <- 1
dum08 <- rep(0, length(data$y))
dum08[75] <- 1
...
plot(one.int) # response of interest rate to interest rate shocks
...
plot(one.gdp) # response of output to interest rate shocks
# positive interest rate shock results in declining in output
...
plot(one.inf) # response of inflation to interest rate shocks
# positive interest rate shock results in declining inflation
...
plot(two.int) # response of interest rate to interest rate shocks
# relatively persistent interest rate shock
...
plot(two.gdp) # response of output to interest rate shocks
# positive interest rate shock results in declining in output
...
plot(two.inf) # response of inflation to interest rate shocks
...
plot.ts(
  cbind(one.inf$irf$i, two.inf$irf$i),
  plot.type = "single",
  col = 4:2,
  ylab = "inflation",
  main = "interest rate shock"
)
