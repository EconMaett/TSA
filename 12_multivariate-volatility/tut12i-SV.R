...
sv.fore <- t(matrix(apply(100*exp(sv.pred/2), 2, quantile, c(0.05, 0.5, 0.95)), nrow=3))
...
res.pf <- read.csv(file="res_pf.csv")  # model that uses particle filter
sv.pf <- 100*res.pf[1:3622,3]

cond.vol <- cbind(garch.h_t, sv.h_t[,2],sv.pf)           

plot.ts(cond.vol, plot.type="single", col = c("blue","green","orange"))
legend("topleft", legend = c("garch","stochvol","particle"), lty = 1, col = c("blue","green","orange"))
...
plot.ts(results, plot.type="single", col = c("red","blue","green"))
legend("topright", legend = c("actual","garch","stochvol"), lty = 1, col = c("red","blue","green"))
