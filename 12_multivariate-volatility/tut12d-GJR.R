...
mod.intel.n <- garchFit(~aparch(1,1),data=intel,trace=F,delta=2,include.delta=F)
summary(mod.intel.n)

mod.intel.s <- garchFit(~aparch(1,1),data=intel,trace=F,delta=2,include.delta=F,cond.dist="std")
summary(mod.intel.s)
...
mod.ibm.n=garchFit(~aparch(1,1),data=ibm,trace=F,delta=2,include.delta=F)
summary(mod.ibm.n)

mod.ibm.s <- garchFit(~aparch(1,1),data=ibm,trace=F,delta=2,include.delta=F,cond.dist="std")
summary(mod.ibm.s)
