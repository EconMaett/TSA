...
m3 <- garchFit(intel~garch(1,1),data=intel,cond.dist="std",trace=F)
summary(m3) # all coefficients appear significant & Qstat is good

# Consider skewed students t-distribution
m4 <- garchFit(intel~garch(1,1),data=intel,cond.dist="sstd",trace=F)
...
m5 <- garchFit(intel~arma(1,0)+garch(1,1),data=intel,trace=F)
