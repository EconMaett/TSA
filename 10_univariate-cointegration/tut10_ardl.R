...
adfcpi <- ur.df(br_month$cpi, type = "none", selectlags = c("BIC"))
summary(adfcpi)
adfmpr <- ur.df(br_month$mpr, type = "none", selectlags = c("BIC"))
summary(adfmpr)
adfreer <- ur.df(na.omit(br_month$reer), type = "none", selectlags = c("BIC"))
summary(adfreer)

# An ARDL(2,1,1) model structure that is used to decribe the monetary policy rate `mpr` with 2 lags
...
m2 <- auto.ardl(mpr~cpi+prod+reer|d_lula, data=br_month, ymax=2, xmax=c(2,2,2), ic="bic")
summary(m2)
